...
/* In this kata, you have an input string and you should check whether it is a valid message. 
   ...
   return true, because "hey" is 3 characters, "hello" is 5, and "hi" is 2; as the numbers and the character
   counts match, the result is true.
...
function isAValidMessage(message) {
  ...
  while (message.length) {
    const num =
      reg.test(message) && !message.search(reg) && message.match(reg)[0];
    message = message.replace(num, "");
    const str = reg.test(message)
      ? message.slice(0, message.search(reg))
      : message;
